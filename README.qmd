...
And these are the available fonts to choose, combine it with `1COL` or `2COL`,
for a one column or two column layout, respectively.

Arial (`Arial`), Garamond (`Garamond`), Helvetica (`Helvetica`), Lato (`LATO`),
Minion Pro (`Minion`), Myriad Pro (`Myriad`), NewcenturySchoolBk (default), Stix
(`STIX`), Times (`Times`), Univers (`Univers`) and Utopia (`Utopia`).
